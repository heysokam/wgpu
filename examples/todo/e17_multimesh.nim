...
#__________________
# Mesh Drawing Shader. The uniform W variable is an array<mat4, 100>
const shaderCode = """
struct Uniforms {
  W     :array<mat4x4<f32>, $1>,
  V     :mat4x4<f32>,
  P     :mat4x4<f32>,
  time  :f32,
  color :vec4<f32>,
...
@vertex fn vert(in :VertIn) ->VertOut {
  ...
  var out   :VertOut;
  out.pos   = u.P * u.V * u.W[in.iid] * vec4<f32>(in.pos, 1.0);
...
# NEW:
# 1. Generate both meshes (Deinterleaved vertex attributes, same layout for both)
var cube    = genCube(size = 1)  # CHG: The cube mesh generation is now coming from the extras file
var pyramid = genPyramid()       # NEW: Generate the pyramid Mesh
...
# Generate the instances World matrices
...
proc run=
  ...
  var requiredLimits = RequiredLimits(
    ...
    limits                : Limits.new(  # With custom `new` proc because some required defaults are not 0
      ...
      maxVertexBuffers    = 2*5,  # NEW: 2xMesh * 5xBuffers
      # Limits for the one uniform value we upload.
      maxBindGroups                   = 1,
  ...
  # NEW:
  # 2. Create the geometry layout for the Mesh objects
  var geoLayout :seq[VertexBufferLayout]  # Was cubeLayouts. Renamed because its setup is not exclusive to the cube
  # Positions
  geoLayout.add VertexBufferLayout(
    arrayStride       : Vec3.size,
  ...
  var offset :uint64
  # 3. Create and upload the cube buffers
  # Change: Separated buffer creation from the Mesh layout setup
  ...
  queue.write(cubeBuffer, offset,   cube.pos[0].addr,   cube.pos.size);  offset += cube.pos.size
  queue.write(cubeBuffer, offset, cube.color[0].addr, cube.color.size);  offset += cube.color.size
  queue.write(cubeBuffer, offset,    cube.uv[0].addr,    cube.uv.size);  offset += cube.uv.size
  queue.write(cubeBuffer, offset,  cube.norm[0].addr,  cube.norm.size);  offset += cube.norm.size
  queue.write(cubeBuffer, offset,  cube.inds[0].addr,  cube.inds.size);  offset  = 0

  # 4. Create and upload the pyramid buffers
  # Change: Separated buffer creation from the Mesh layout setup
  var pyramidBuffer = device.create(vaddr BufferDescriptor(
    nextInChain       : nil,
    label             : "pyramid Vertex buffer".cstring,
    usage             : {BufferUsage.copyDst, BufferUsage.vertex, BufferUsage.index},
    size              : pyramid.size,
    mappedAtCreation  : false,
    )) # << device.createBuffer()
  # Write the indices to the buffer
  queue.write(pyramidBuffer, offset,   pyramid.pos[0].addr,   pyramid.pos.size);  offset += pyramid.pos.size
  queue.write(pyramidBuffer, offset, pyramid.color[0].addr, pyramid.color.size);  offset += pyramid.color.size
  queue.write(pyramidBuffer, offset,    pyramid.uv[0].addr,    pyramid.uv.size);  offset += pyramid.uv.size
  queue.write(pyramidBuffer, offset,  pyramid.norm[0].addr,  pyramid.norm.size);  offset += pyramid.norm.size
  queue.write(pyramidBuffer, offset,  pyramid.inds[0].addr,  pyramid.inds.size);  offset  = 0

  # OLD: Create the Uniform Buffer that will hold the variable, and upload the initial data
  ...
  let pipeline = device.create(vaddr RenderPipelineDescriptor(
    ...
    vertex                   : VertexState(
      ...
      constantCount          : 0,
      constants              : nil,
      bufferCount            : geoLayout.len.uint32,
      buffers                : geoLayout[0].addr,
      ), # << vertex
  ...
  while not window.close():
    ...
    # NEW:
    # 3. Setup and draw the cube  (Change: We will setup both objects, instead of only the cube )
    # Set the vertex attributes buffer
    renderPass.setVertexBuffer(Attr.pos,   cubeBuffer, offset,   cube.pos.size);  offset += cube.pos.size
    renderPass.setVertexBuffer(Attr.color, cubeBuffer, offset, cube.color.size);  offset += cube.color.size
    renderPass.setVertexBuffer(Attr.uv,    cubeBuffer, offset,    cube.uv.size);  offset += cube.uv.size
    renderPass.setVertexBuffer(Attr.norm,  cubeBuffer, offset,  cube.norm.size);  offset += cube.norm.size
    # Set the Index block of the buffer in the renderPass
    renderPass.setIndexBuffer(cubeBuffer, IndexFormat.Uint32, offset, cube.inds.size);  offset = 0
    # Draw all of the cubes instanced
    ...
    # 4. Setup and draw the pyramid
    # Set the vertex attributes buffer
    renderPass.setVertexBuffer(Attr.pos,   pyramidBuffer, offset,   pyramid.pos.size);  offset += pyramid.pos.size
    renderPass.setVertexBuffer(Attr.color, pyramidBuffer, offset, pyramid.color.size);  offset += pyramid.color.size
    renderPass.setVertexBuffer(Attr.uv,    pyramidBuffer, offset,    pyramid.uv.size);  offset += pyramid.uv.size
    renderPass.setVertexBuffer(Attr.norm,  pyramidBuffer, offset,  pyramid.norm.size);  offset += pyramid.norm.size
    # Set the Index block of the buffer in the renderPass
    renderPass.setIndexBuffer(pyramidBuffer, IndexFormat.Uint32, offset, pyramid.inds.size);  offset = 0
    # Draw the pyramid. Will use the u.W matrix at id `firstInstance`, since we are ordering to draw 1 instance
    renderPass.draw(pyramid.indsCount, 1, 0,0,0)  # vertexCount, instanceCount, firstVertex, baseVertex, firstInstance
