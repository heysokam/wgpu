...
proc run=
  ...
  # NEW:
  # 3. Specify the bindGroup as accesible by the vertex and fragment stages
  # 4. Mark the minimum binding size as the size of our Uniforms object
  var bindGroupLayout = device.create(vaddr BindGroupLayoutDescriptor(
    ...
    entries              : vaddr BindGroupLayoutEntry(  # Uniform Layout Entry starts here
      ...
      buffer             : BufferBindingLayout(
        ...
        minBindingSize   : uint64( sizeof(Uniforms) ),  # NEW: Minimum size for uploaded values is now the size of the object
        ), # << buffer
  ...
  # Create the PipelineLayout, with our bindgroup as input
  ...
  # Create the BindGroup, with our single uniform variable.
  # NEW: 5. The size of the gpu buffer is now the size of our Uniforms object
